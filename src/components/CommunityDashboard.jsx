...
export function CommunityDashboard({ supabase, session, profile }) {
  ...
  const conversationUsers = [...new Set(messages.flatMap((message) => [message.sender_id, message.receiver_id]).filter((id) => id !== session.user.id))]
  const conversationMessages = (userId) => messages.filter((message) => message.sender_id === userId || message.receiver_id === userId)
  const openConversation = async (userId) => {
    setSelectedFriend(userId)
    await supabase.rpc('mark_messages_read', { conversation_user_id: userId })
    setMessages((current) => current.map((message) => message.sender_id === userId && message.receiver_id === session.user.id ? { ...message, is_read: true } : message))
  }
